...
import { fetchMaintenanceSchedulesAction } from '@/app/lib/actions';

import { MaintenanceAnalyticsDetailSkeleton } from '@/app/ui/skeletons';

import MaintenanceMetricCard from '@/components/analytics/MetricCard';

function MaintenanceAnalyticsContent({ maintenanceData }: { maintenanceData: any[] }) {
    ...
    <div className="w-full min-h-screen bg-[#0a0514] text-white font-mono p-6 lg:p-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <MaintenanceMetricCard
          ...
          value={maintenanceData.filter((m) => m.status === 'DONE').length.toString()}
                ...
                {paginatedMaintenanceData.map((ship, i) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-6">
                      <span
                        className={`text-[9px] px-3 py-1.5 rounded-lg border font-black tracking-widest ${
                          ...
                          <div
                            className={`h-full transition-all duration-1000 ${
                              ship.status === 'CRITICAL'
                                ...
                                : 'bg-[#bc66ff] shadow-[0_0_8px_#bc66ff]'
                            }`}
          ...
          <div className="bg-[#150e24]/60 border border-white/5 rounded-[2.5rem] p-8 backdrop-blur-md">
            ...
            <div className="space-y-6">
              {statusSummary.map((stat, i) => {
                const count = maintenanceData.filter(
                  (m) => m.status === stat.label
                ).length;

                const perc =
                  totalVessels > 0 ? (count / totalVessels) * 100 : 0;
...
export default function MaintenanceAnalytics() {
  ...
  useEffect(() => {
    fetchMaintenanceSchedulesAction().then((rows) => {
      setMaintenanceData((rows as any[]).map((item) => ({
        name: item.vehicle_name,
        status: item.status === 'IN_PROGRESS' ? 'ACTIVE' : item.status === 'PLANNED' ? 'PLANNED' : 'DONE',
        progress: item.status === 'DONE' ? 100 : item.status === 'IN_PROGRESS' ? 55 : 20,
        nextService: String(item.maintenance_date).slice(0, 10),
        failure: item.task,
        eta: String(item.maintenance_date).slice(0, 10),
        engineEff: item.task,
      })));
      setIsLoading(false);
  ...
  return (
    <Suspense fallback={<MaintenanceAnalyticsDetailSkeleton />}>
      {isLoading ? (
        <MaintenanceAnalyticsDetailSkeleton />
      ) : (
        <MaintenanceAnalyticsContent maintenanceData={maintenanceData} />
      )}
    </Suspense>
  );
}
