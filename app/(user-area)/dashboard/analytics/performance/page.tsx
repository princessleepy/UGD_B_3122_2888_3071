...
import { fetchShipmentPerformanceAction } from '@/app/lib/actions';
...
function PerformanceContent({ performanceData }: { performanceData: any[] }) {
  const [currentPage, setCurrentPage] = useState(1);

  const itemsPerPage = 5;

  const vesselBreakdown = performanceData.map((v) => {
    const score = v.trips > 0
      ? Math.max(0, Math.min(100, Math.round(((v.trips - v.delayed) / v.trips) * 100)))
      : 0;
    ...
    return {
      ...
      score,
      trips: v.trips,
      speed: `${Number(v.total_weight || 0).toLocaleString('id-ID')} KG`,
      status: v.delayed > 0 || v.vehicle_status === 'MAINTENANCE' ? 'MAINTENANCE' : 'OPTIMAL',
    };
  });
  ...
  const avgEfficiency =
    vesselBreakdown.length > 0
      ? Math.round(
          vesselBreakdown.reduce(
            (acc, curr) => acc + curr.score,
            0
          ) / vesselBreakdown.length
        )
      : 0;
      ...
      <div className="px-10 pt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <MaintenanceMetricCard
            ...
            value={vesselBreakdown.length.toString()}
            status="UNITS"
            progress={100}
          />
          ...
          <MaintenanceMetricCard
            title="System Alerts"
            value={vesselBreakdown
              .filter((m) => m.status === 'MAINTENANCE')
              .length.toString()}
            status="ISSUES"
            statusType={
              vesselBreakdown.some((m) => m.status === 'MAINTENANCE')
                ? 'warning'
                : 'optimal'
            }
          />
...
export default function PerformancePage() {
  ...
  useEffect(() => {
    fetchShipmentPerformanceAction().then((rows) => {
      setPerformanceData(rows as any[]);
      setLoading(false);
