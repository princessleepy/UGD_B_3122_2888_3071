...
import { VehicleStats, VesselAuditRow } from '@/app/lib/definitions';
...
function getFuelColor(status: string) {
...
export default function AnalyticsDashboardClient({
  ...
  vesselAudit: VesselAuditRow[];
  chartData: number[];
  chartLabels: string[];
}) {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 5;
  const totalPages = Math.ceil(vesselAudit.length / itemsPerPage) || 1;
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedAuditData = vesselAudit.slice(
    startIndex,
    ...
    <div className="w-full bg-[#0a0514] min-h-screen">
      <div className="px-10 pt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <MetricCard title="FLEET READINESS" value={`${stats.readiness}%`} />
          <MetricCard
          ...
          <MetricCard
            ...
            subtitle={
              stats.maintenance > 0 ? 'ACTION REQUIRED' : 'SYSTEM CLEAR'
            }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <div className="lg:col-span-2 bg-[#150e24] rounded-[2.5rem] border border-white/5 overflow-hidden backdrop-blur-xl shadow-2xl min-h-[400px] flex flex-col">
            <ChartBox chartData={chartData} chartLabels={chartLabels} />
          </div>
          <div className="bg-[#150e24] rounded-[2.5rem] border border-white/5 p-6 flex items-center justify-center shadow-2xl">
            <DonutChart
              ...
              plannedCount={statusDistribution.planned}
            />
          </div>
        </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="text-[9px] text-white/30 uppercase tracking-[0.25em] border-b border-white/5">
                  <th className="px-8 py-6 font-black">Vessel Identifier</th>
                  <th className="px-8 py-6 font-black">Fuel Status</th>
                  <th className="px-8 py-6 font-black">Cons. Rate</th>
                  <th className="px-8 py-6 font-black">Voyage Dist.</th>
                  <th className="px-8 py-6 font-black text-right">Efficiency Score</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {paginatedAuditData.map((item) => {
                  const label = getEfficiencyLabel(item.efficiency_score);
                  return (
                    <tr
                      key={item.id}
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        <div className="w-32 h-1.5 bg-black/40 rounded-full overflow-hidden">
                          <div
                            className={`h-full ${getFuelColor(item.status)}`}
                            style={{ width: `${item.fuel_percentage}%` }}
                          />
                        </div>
                        <p className="text-[8px] text-white/40 font-black uppercase mt-2">
                          {item.fuel_percentage}% FUEL
                        </p>
                      </td>
                      <td className="px-8 py-6 text-[11px] text-white font-black">
                        {item.consumption_rate} MT/DAY
                      </td>
                      <td className="px-8 py-6 text-[11px] text-white font-black">
                        {item.voyage_distance} NM
                      </td>
