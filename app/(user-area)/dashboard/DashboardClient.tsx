    ...
    <div className="min-h-screen bg-[#0d0415] text-white p-6 font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        {dashboardStats.map((stat, i) => (
          <div
            key={i}
            className="bg-[#1a0b2e] p-5 rounded-[20px] border border-white/5 shadow-lg"
          >
            <p className="text-[9px] tracking-[0.2em] text-gray-500 mb-2 uppercase font-bold">
              {stat.label}
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2">
          <h2 className="text-[13px] font-extrabold tracking-[0.25em] mb-4 text-white uppercase italic">
            Global Map Feed
          </h2>
          <div className="bg-[#1a0b2e] rounded-[2.5rem] border border-white/5 relative h-[450px] overflow-hidden group shadow-2xl">
            <div className="absolute top-6 left-6 z-20 bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 flex items-center gap-2">
              ...
              <span className="text-[9px] tracking-widest font-bold text-white uppercase">
                Live Telemetry // Signal Active
              </span>
            </div>
            ...
            <div className="absolute bottom-8 right-8 z-20 text-right bg-black/60 backdrop-blur-xl p-6 rounded-[2rem] border border-white/10 shadow-2xl">
              <p className="text-[7px] text-gray-500 uppercase font-black tracking-[0.2em] mb-1">
                Target Signal
              </p>
              <p className="text-sm font-black text-[#bc66ff] tracking-tight uppercase">
                {vehicles[0]?.vehicle_name ?? 'NO VESSEL'}
              </p>
              <p className="text-[9px] text-gray-400 font-bold mt-1 uppercase">
                ID: {vehicles[0]?.vehicle_code ?? '-'}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-[#1a0b2e] rounded-[2.5rem] border border-white/5 p-6 flex-grow shadow-lg overflow-y-auto max-h-[450px]">
            <div className="space-y-4">
              {alerts.map((alert, idx) => (
                <div
                  key={idx}
                  className={`flex gap-4 p-4 border-l-2 rounded-r-2xl ${
                    alert.color === 'rose'
                      ? 'bg-rose-500/5 border-rose-500'
                      : 'bg-[#bc66ff]/5 border-[#bc66ff]'
                  }`}
                >
                  {alert.color === 'rose' ? (
      ...
      {/* Fleet Registry Table + Performance */}
          ...
          <table className="w-full text-left border-collapse">
            <thead className="text-[9px] text-gray-500 uppercase tracking-[0.2em] bg-white/5">
              <tr>
                ...
                <th className="px-8 py-4 font-black">Capacity</th>
              </tr>
            </thead>
                ...
                >
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-2">
                      <div className={`w-1.5 h-1.5 rounded-full ${getStatusColor(v.status).replace('text-', 'bg-')}`} />
                      <span className={`text-[9px] font-black uppercase tracking-tight ${getStatusColor(v.status)}`}>
                        {v.status}
                      </span>
                    </div>
                  </td>
                  <td className="px-8 py-5 text-[10px] font-black text-white uppercase">
                    {v.vehicle_type}
                  </td>
              ...
              {[45, 65, 50, 95, 70, 55, 85].map((h, i) => (
                <div
                  ...
                  className={`w-full rounded-t-lg transition-all duration-500 hover:brightness-125 ${
                    i === 3 ? 'bg-[#bc66ff] shadow-[0_0_20px_#bc66ff]' : 'bg-white/10'
                  }`}
                  style={{ height: `${h}%` }}
                />
              ))}
      ...
      <Link href="/dashboard/fleet/map" className="hidden">Map</Link>
    </div>
  );
}